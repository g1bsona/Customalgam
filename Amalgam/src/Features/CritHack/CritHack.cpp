...
void CCritHack::Draw(CTFPlayer* pLocal)
{
	...
	if (!pWeapon->AreRandomCritsEnabled())
	{
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, "Random crits disabled");
		else
	...
	if (Vars::Misc::Game::AntiCheatCompatibility.Value)
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, "Anticheat compatibility");
		else
	...
	if (pLocal->IsCritBoosted())
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextMisc.Value, align, "Crit Boosted");
		else
	...
	else if (pWeapon->m_flCritTime() > flTickBase)
	{
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextMisc.Value, align, std::format("Streaming crits {:.1f}s", flTime).c_str());
		else
	...
	else if (!m_bCritBanned)
	{
		if (m_iPotentialCrits > 0)
		{
			if (m_iAvailableCrits > 0)
			{
				if (!pWeapon->IsRapidFire() || flTickBase >= pWeapon->m_flLastRapidFireCritCheckTime() + 1.f)
					if (!Vars::Menu::StringOutlined.Value)
						H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextGood.Value, align, "Crit Ready");
					else
				...
				else
				{
					...
					if (!Vars::Menu::StringOutlined.Value)
						H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("Wait {:.1f}s", flTime).c_str());
					else
			...
			else
			{
				...
				if (!Vars::Menu::StringOutlined.Value)
					H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, std::format("Crit in {}{} shot{}", iShots, iShots == BUCKET_ATTEMPTS ? "+" : "", iShots == 1 ? "" : "s").c_str());
				else
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextBad.Value, align, "Crit banned");
		else
	...
	if (m_iPotentialCrits > 0)
	{
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("{}{} / {} crits", iCrits, iCrits == BUCKET_ATTEMPTS ? "+" : "", m_iPotentialCrits).c_str());
		else
		...
		if (m_iNextCrit && iCrits)
		{
			int iShots = m_iNextCrit;
			if (!Vars::Menu::StringOutlined.Value)
				H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("Next in {}{} shot{}", iShots, iShots == BUCKET_ATTEMPTS ? "+" : "", iShots == 1 ? "" : "s").c_str());
			else
	...
	if (m_flDamageTilFlip && !m_bCritBanned)
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Colors::IndicatorTextGood.Value, align, std::format("{} damage til flip", ceilf(m_flDamageTilFlip)).c_str());
		else
	...
	if (m_iDesyncDamage)
	{
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, tColor, align, std::format("{}{} desync", m_iDesyncDamage > 0 ? "+" : "", m_iDesyncDamage).c_str());
		else
	...
	{
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall * 2, Vars::Menu::Theme::Active.Value, align, std::format("MeleeDamage: {}", m_iMeleeDamage).c_str());
		else
		...
		if (s_pCTFGameStats)
		{
			if (auto pPlayer2 = S::UTIL_PlayerByIndex.Call<void*>(I::EngineClient->GetLocalPlayer()))
			{
				if (auto pPlayerStats = S::CTFGameStats_FindPlayerStats.Call<PlayerStats_t*>(s_pCTFGameStats, pPlayer2))
				{
					...
					if (!Vars::Menu::StringOutlined.Value)
						H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("RangedDamage: {}, CritDamage: {}", iRangedDamage, iCritDamage).c_str());
					else
					...
					if (!Vars::Menu::StringOutlined.Value)
						H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("AllDamage: {} ({})", iDamage, iDamage - iRangedDamage).c_str());
					else
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("ResourceDamage: {} ({})", m_iResourceDamage, m_iMeleeDamage).c_str());
		else
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall * 2, Vars::Menu::Theme::Active.Value, align, std::format("Bucket: {}, Shots: {}, Crits: {}", pWeapon->m_flCritTokenBucket(), pWeapon->m_nCritChecks(), pWeapon->m_nCritSeedRequests()).c_str());
		else
		...
		if (!Vars::Menu::StringOutlined.Value)
			H::Draw.String(fFont, x, y += nTall, Vars::Menu::Theme::Active.Value, align, std::format("Damage: {}, Cost: {}", m_flDamage, m_flCost).c_str());
		else
